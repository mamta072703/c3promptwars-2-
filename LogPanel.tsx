...
import { Sparkles, Plus, Trash2, ShieldAlert, CheckCircle, Flame, Car, Plane, Home, Salad, ArrowRight, Loader2, Droplets, Trash } from "lucide-react";
import { CarbonLog } from "../types";
import { calculateImpact, getComparativeEquivalencePhrase } from "../utils/carbonCalculator";
...
export default function LogPanel({
  ...
  const [manualSubtype, setManualSubtype] = useState("beefServing");
  ...
  const handleCategoryChange = (cat: CarbonLog["category"]) => {
    setManualCategory(cat);
    if (cat === "diet") {
      setManualSubtype("beefServing");
    } else if (cat === "travel") {
      setManualSubtype("gasCarKm");
    } else if (cat === "energy") {
      setManualSubtype("electricityKwh");
    } else {
      setManualSubtype("foodScraps");
    }
  };
  ...
  const handleManualSubmit = (e: React.FormEvent) => {
    ...
    // Use our state-of-the-art robust calculator
    const impact = calculateImpact(
      manualCategory === "other" ? "waste" as any : manualCategory,
      manualSubtype,
      numericValue
    );

    const comparison = getComparativeEquivalencePhrase(impact.carbonKg);

    // Format a thorough title referencing the metric
    const formattedName = `${manualName} [${numericValue}x ${manualSubtype}]`;

    // Package the manual log
    onAddManualLog({
      name: formattedName,
      category: manualCategory,
      carbonFootprintKg: impact.carbonKg,
      explanation: `${impact.explanation} Water impact: ${impact.waterLitres} Litres. Physical waste byproduct: ${impact.wasteKg} kg.`,
      ...
      <div className="lg:col-span-5 space-y-6">
        ...
        <div className="bg-gradient-to-br from-indigo-900 to-slate-900 text-white rounded-2xl p-5 border border-slate-800 shadow-md relative overflow-hidden">
          ...
          <form onSubmit={handleAiScan} className="space-y-3">
            <label htmlFor="ai-scan-textarea" className="sr-only">Describe transport, diet, or utility behaviors for AI Carbon computing</label>
            <textarea
              id="ai-scan-textarea"
              value={aiText}
              ...
              className="w-full text-xs bg-slate-800 text-white border border-slate-700 rounded-xl p-3 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent resize-none"
              aria-label="AI scan query text input"
            />
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
          ...
          <form onSubmit={handleManualSubmit} className="space-y-4">
            <div>
              <label htmlFor="manual-entry-name" className="block text-xs font-medium text-gray-700 mb-1.5 font-sans">
                Item / Activity Name
              </label>
              <input
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label htmlFor="manual-category-select" className="block text-xs font-medium text-gray-750 mb-1.5 flex items-center justify-between">
                  <span>Category</span>
                </label>
                <select
                  id="manual-category-select"
                  value={manualCategory}
                  onChange={(e) => handleCategoryChange(e.target.value as CarbonLog["category"])}
                  className="w-full text-xs font-medium border border-gray-200 rounded-xl p-2.5 focus:outline-none focus:ring-1 focus:ring-emerald-500 bg-gray-50/50"
                >
                  <option value="diet">Diet / Food</option>
                  <option value="travel">Travel / Transit</option>
                  <option value="energy">Home Energy</option>
                  <option value="other">Consumption & Waste</option>
                </select>
              </div>

              <div>
                <label htmlFor="manual-subtype-select" className="block text-xs font-medium text-gray-750 mb-1.5 font-sans">
                  Action Subtype
                </label>
                <select
                ...
                >
                  {manualCategory === "diet" && (
                    <>
                      <option value="beefServing">🥩 Beef portion (300g)</option>
                      <option value="chickenServing">🍗 Poultry portion (300g)</option>
                      <option value="fishServing">🐟 Seafood portion (300g)</option>
                      <option value="vegetarianServing">🍳 Vegetarian portion</option>
                      <option value="veganServing">🌱 Complete Vegan plant serving</option>
                      <option value="dairyServing">🥛 Standard Dairy glass/cheese</option>
                    </>
                  )}
                  {manualCategory === "travel" && (
                    <>
                      <option value="gasCarKm">🚗 Gasoline Car (per km)</option>
                      <option value="hybridCarKm">🚙 Hybrid Car (per km)</option>
                      <option value="electricCarKm">⚡ Electric Car EV (per km)</option>
                      <option value="busKm">🚌 Public Transit Bus (per km)</option>
                      <option value="trainKm">🚆 Rail Train Journey (per km)</option>
                      <option value="bikeKm">🚲 Cycling standard commute (per km)</option>
                      <option value="flightKm">✈️ Commercial Plane Flight (per km)</option>
                    </>
                  )}
                  {manualCategory === "energy" && (
                    <>
                      <option value="electricityKwh">🔌 Standard Grid Electricity (kWh)</option>
                      <option value="gasKwh">🔥 Natural Fossil Gas (kWh)</option>
                      <option value="greenEnergyKwh">🍃 Clean Solar/Wind (kWh)</option>
                    </>
                  )}
                  {manualCategory === "other" && (
                    <>
                      <option value="foodScraps">🍎 Mixed Organic Scraps (kg)</option>
                      <option value="plasticMaterial">🥤 Synthetics & Plastic (kg)</option>
                      <option value="paperCardboard">📦 Cardboard Packaging (kg)</option>
                    </>
                  )}
                </select>
              </div>
            ...
            <div>
              <label htmlFor="manual-quantity-input" className="block text-xs font-medium text-gray-750 mb-1.5">
                Quantity
              </label>
              <div className="relative">
                <input
                  ...
                  placeholder="E.g. 5"
                  className="w-full text-xs border border-gray-200 rounded-xl p-2.5 pr-20 focus:outline-none focus:ring-1 focus:ring-emerald-500 bg-gray-50/50"
                  aria-label="Measurement quantity value"
                />
                <div className="absolute right-3 top-2.5 text-[10px] uppercase font-bold text-gray-400 pointer-events-none">
                  ...
                  {manualCategory === "energy" && "kWh"}
                  {manualCategory === "other" && "kg"}
                </div>
              </div>
            </div>
            ...
            <button
              ...
              className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold transition-colors active:scale-[0.99] flex items-center justify-center gap-1.5 cursor-pointer"
            >
