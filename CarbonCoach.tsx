  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex flex-col h-[520px]">
      ...
      <form onSubmit={onSubmit} className="p-3 bg-slate-50 border-t border-gray-100 flex gap-2">
        <label htmlFor="coach-speech-input" className="sr-only">Ask Carbon coach a question</label>
        <input
          id="coach-speech-input"
          type="text"
          value={userInput}
          disabled={isSending}
          ...
          aria-label="Coach message text"
        />
        <button
          type="submit"
          disabled={!userInput.trim() || isSending}
          aria-label="Submit coach question"
          className="w-10 h-10 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white shrink-0 flex items-center justify-center transition-colors disabled:opacity-40 disabled:pointer-events-none active:scale-95 cursor-pointer"
        >
